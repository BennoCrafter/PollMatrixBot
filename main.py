...
def clean_up_session_file(session_file_path: Path, should_delete: bool):
    """Delete session.txt file if required by configuration."""
    if session_file_path.exists() and should_delete:
...
async def close_poll(room):
    ...
    if poll:
        await bot.api.send_markdown_message(
            room.room_id, poll.formated_markdown(f"## {config['status_title']} {poll.name} (closed):")
        )
...
async def list_items(room):
    ...
    if poll:
        await bot.api.send_markdown_message(
            room.room_id, poll.formated_markdown(f"## {config['status_title']} {poll.name}:")
        )
...
async def add_item_to_poll(room, message, match, config):
    ...
    body_msg = (
        message.body.strip() if not config["use_add_command"]
        else ' '.join(match.args()).strip()
    )
...
if __name__ == "__main__":
    ...
    clean_up_session_file(session_file_path, config.get("delete_session_file_on_start"))
